...
const ProductAdminPage:NextPage<Props> = ({ product }) => {
    ...
    const notify = (message: string, type: VariantType, slug?: string) => {
        ...
        if(slug) router.push(`/admin/products/${slug}`)
    }
    ...
    const onDeleteImage = async(url:string) => {
        ...
        try {
            await shopApi.delete(`/files/images`, {
                data: { filename }
            });
            const currentImages = getValues('images');
            const updatedImages = currentImages.filter((image: string) => image !== url);
            setValue('images', updatedImages, { shouldValidate: true });
            notify('Imágen eliminada correctamente', 'success');
        } catch (e) {
    ...
    const onSubmit = async( form: FormData ) => {
        ...
        try {
            ...
            const { images, id, created_at, updated_at, ...restForm } = form
            const process_data = {
                ...restForm,
                price: Number(form.price),
                stock: Number(form.stock),
            }
            await shopApi.patch(`/products/${product.id}`, process_data);
            const { data } = await shopApi.post(`/files/product/${form.id}`, formData);
            setValue('images', [...getValues('images'), ...data.images], { shouldValidate: true });
            setSelectedFiles([]);
            setSelectedImages([]);
            setIsSaving(false);
        ...
        >
            <form onSubmit={ handleSubmit( onSubmit ) }>
                <div className='flex justify-end my-4'>
                    <button
                        className='py-2 px-5 rounded-full bg-indigo-600 text-white'
                        type="submit"
                        disabled={ isSaving }
                    >
                        {
                ...
                <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
                    ...
                    <div className='flex flex-col space-y-4'>
                        ...
                        <div className='relative flex flex-col'>
                            <label className='my-2 dark:text-white text-black font-semibold'>Imágenes: </label>
                            <div className='relative'>
                                <ImageUpload 
                            ...
                            <div className='relative flex flex-col mt-4 space-y-2'>
                                <label className='dark:text-white text-black font-semibold'>Imagenes subidas: </label>
                                <div className='flex flex-wrap'>
                                    {
                                        getValues('images').map((img, i) => (
                                            <div key={i} className='w-[150px] ml-4 my-1 flex flex-col'>
                                                <div>
                                                    <img
                                                        className='fadeIn h-[150px] object-cover'
                                                        src={ img }
                                                        alt={ img }
                                                    />
                                                </div>
...
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
    ...
    const { data } = await shopApi.get(`/products/${slug.toString()}`);
    product = data;
    if ( !product ) {
        return {
            redirect: {
